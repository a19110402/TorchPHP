...
function getDate(){
  ...
  let dhlShipperPostalCodeValidation = false;
  let dhlRecipientPostalCodeValidation = false;
...
//EVALUATING EVERY TIME THERE IS A CHANGE
  $("#dhl_shipper_postalCode").on('change',function(e){
    $("#dhl_shipper_postalCode").css("border-color", "black");
    $("#shipperPostalCodeNotFound").remove();
    let shipperPostalCode = $('input[name="dhlShipperPostalCode"]').val();
    let shipperCountrCode = $('select[name="dhlShipperCountryCode"]').val();
    ...
    let myPromise = new Promise(function(resolve){
      $("#dhl_shipper_postalCode").css("border", "2px solid #33AFFF"); 
    ...
    myPromise.then(function(response){
      switch(response.fedexResponse.statusCode){
          case 200:
            $("#dhl_shipper_postalCode").css("border", "2px solid #8bef89");  
            dhlShipperPostalCodeValidation = true;
            break;
          case 400:
            $("#dhl_shipper_postalCode").css("border", "2px solid red").after("<p id='shipperPostalCodeNotFound'>No pudimos encontrar el código postal</p>");  
            $("#shipperPostalCodeNotFound").css("font-size", "1rem");
            dhlShipperPostalCodeValidation = false;
          }
    });
  });
  
  $("#dhl_recipient_postalCode").on('change',function(e){
    $("#dhl_recipient_postalCode").css("border-color", "black");
    let recipientPostalCode = $('input[name="dhlRecipientPostalCode"]').val();
    let recipientCountryCode = $('select[name="dhlRecipientCountryCode"]').val();
  ...
  let myPromise = new Promise(function(resolve){
    $("#dhl_recipient_postalCode").css("border", "2px solid #33AFFF");  
    $("#validatingPostalCode").css('font-size', "1rem");
    let response = validatePostalCode(postalCodeAPI);
    resolve(response);
  });
  ...
  myPromise.then(function(response){
    switch(response.fedexResponse.statusCode){
        case 200:
          $("#dhl_recipient_postalCode").css("border", "2px solid #8bef89");  
          dhlRecipientPostalCodeValidation = true;
          break;
        case 400:
          $("#dhl_recipient_postalCode").css("border", "2px solid red").after("<p id='recipientPostalCodNotFound'>No pudimos encontrar el código postal</p>");   
          $("#recipientPostalCodeNotFound").css("font-size", "1rem");
          dhlRecipientPostalCodeValidation = false;
        }
  });
  });
...
//   });
  ...
  export {dhlShipperPostalCodeValidation, dhlRecipientPostalCodeValidation, getDate};
